 ...
 * 种子脚本（开发 / 一次性修复用）。
 *
 * 本文件导出两类可复用函数，便于「自举种子」在应用启动期幂等执行
 * （见 `seed/bootstrap-seed.ts`），而无需在每次部署都跑一次性 `npm run seed`：
 * - `ensureProviderConfigs(ds)`：幂等确保系统 provider 配置（按 name 检查，绝不 clear）。
 * - `seedContent(ds)`：插入课程/单词/句子/任务等初始内容（纯插入，无 clear）。
 *
 * `seed()` 仍保留「clear + 全量重播种」的 CLI 行为（用于重置本地/演示库），
 * 且仅在作为脚本直接执行时自动运行（`require.main === module`），被 import 时不触发。
 */
...
/**
 * 插入初始内容（课程 / 单词 / 句子 / 每日任务）。**纯插入、无 clear**，
 * 因此可安全在启动期调用；调用方（bootstrap-seed）负责「仅空表时播种」避免重复。
 */
export async function seedContent(ds: DataSource): Promise<void> {
...
/**
 * CLI 全量重置：clear 内容表 → 重播种内容 → 确保 provider 配置 → 关闭连接。
 * 仅在 `npm run seed`（脚本直接执行）时自动运行；被 import 时不触发。
 */
...
// 仅作为脚本直接执行时运行（被 import 时（如 bootstrap-seed）不自动跑）。
